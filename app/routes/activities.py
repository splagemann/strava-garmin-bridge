    ...
    try:
        ...
        for activity in activities:
            ...
            if activity.moving_time:
                moving_time = int(activity.moving_time.total_seconds()) if hasattr(activity.moving_time, 'total_seconds') else int(activity.moving_time)

            # Convert elapsed time to seconds
            elapsed_time = None
            if activity.elapsed_time:
                elapsed_time = int(activity.elapsed_time.total_seconds()) if hasattr(activity.elapsed_time, 'total_seconds') else int(activity.elapsed_time)

            # Convert elevation gain to float
            elevation_gain = None
            if activity.total_elevation_gain:
                elevation_gain = activity.total_elevation_gain.num if hasattr(activity.total_elevation_gain, 'num') else float(activity.total_elevation_gain)
